...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Widget build(BuildContext context) {
    // Matches the logo's own cream backdrop (baked into the SVG itself) so
    // there's no visible seam around it — see assets/logo/rasoi_care_logo.svg.
    return const Scaffold(
      backgroundColor: Color(0xFFEFE7D5),
      body: Center(
        child: Padding(
...
class _SplashLogoState extends State<_SplashLogo> {
  ...
  void initState() {
    super.initState();
    // Fade in on first frame rather than popping in instantly.
  ...
  Widget build(BuildContext context) => AnimatedOpacity(
        ...
        child: SvgPicture.asset(
          'assets/logo/rasoi_care_logo.svg',
          semanticsLabel: 'Rasoi Care',
        ),
      );
}
...
class _PhoneScreenState extends ConsumerState<PhoneScreen> {
  ...
  Future<void> _signInWithGoogle() async {
    ...
    try {
      ...
      if (!mounted) return;
      // Google already verifies name + email — skip straight past OTP and
      // manual registration, matching real OAuth behaviour.
      ScaffoldMessenger.of(context).showSnackBar(
