...
/// The app's [GoRouter], gated by [currentUserRoleProvider]. Built once (this
/// is a plain `Provider`, not `watch`ed against role changes) so navigation
/// state survives sign-in/out; `redirect` re-reads the role fresh on every
/// navigation instead, which is what go_router expects for auth-style gates.
final routerProvider = Provider<GoRouter>((ref) {
  String? guard(BuildContext context, GoRouterState state) {
    final path = state.matchedLocation;
    final staffOnly = path.startsWith('/admin') || path.startsWith('/tech');
    if (!staffOnly) return null;
    final role = ref.read(currentUserRoleProvider);
    final allowed = path.startsWith('/admin')
        ? role == UserRole.admin
  ...
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (_, s) => ServiceDetailScreen(
            appliance: _appliance(s.pathParameters['appliance'])),
      ),
      ...
      GoRoute(
          ...
          parentNavigatorKey: _rootKey,
          builder: (_, s) =>
              ConfirmedScreen(bookingId: s.pathParameters['id']!)),
      GoRoute(
      ...
      StatefulShellRoute.indexedStack(
        ...
        branches: [
          ...
          StatefulShellBranch(
            routes: [
              GoRoute(
                  path: '/services', builder: (_, __) => const CatalogScreen())
            ],
          ),
          StatefulShellBranch(
            routes: [
              GoRoute(
                  path: '/bookings', builder: (_, __) => const BookingsScreen())
            ],
          ),
          StatefulShellBranch(
            routes: [
